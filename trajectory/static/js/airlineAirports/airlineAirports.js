...
function loadOneAirport( globus, airport , showHide ) {
	...
	if (! layer) {
		...
		layerAirport = new og.layer.Vector(layerName , { clampToGround: true, 	});
...
function loadAirports( globus, dataJson , showHide ) {
	...
	for (var airportId = 0; airportId < airports.length; airportId++ ) {
		// insert one airport
		loadOneAirport(  globus, airports[airportId] , showHide );
	}
}

function showHideAllAirports( globus, showHide ) {
...
function initAirports(globus) {
    ...
    document.getElementById("btnAirports").onclick = function () {
        ...
        if (show) {
			...
			showHideAllAirports( globus, true );
			...
			showHideAllAirports( globus, false );
